...
def get_kind_stock_issue(stock_code, company_name, days=1):
    ...
    try:
        ...
        for row in rows:
            ...
            if len(cols) >= 5:
                list_date = cols[0].get_text(strip=True)
                list_type = cols[1].get_text(strip=True)
                ...
                if link_tag and link_tag.get("onclick"):
                    ...
                    if "bzProcsNo" in onclick or "fn_click" in onclick:
                        import re
                        match = re.search(r"'([^']+)'", onclick)
                        if match:
                            bz_no = match.group(1)
                            link = "https://kind.krx.co.kr/corpgeneral/stockissuelist.do?method=loadDetailPage&bzProcsNo=" + bz_no
                if not link:
