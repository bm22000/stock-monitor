...
def build_html(prices, disclosures, all_news, kind_issues):
    ...
    if kind_issues:
        ...
        for k in kind_issues:
            kind_rows += "<tr><td><b>{company}</b></td><td>{date}</td><td>{type}</td><td style='text-align:right'>{shares}</td><td>{reason}</td></tr>".format(**k)
        kind_section = "<h2 style='color:#222;margin-top:36px'>KIND 주식발행내역 (증자/소각 등)</h2><table style='width:100%;border-collapse:collapse;font-size:13px'><tr style='background:#f4f4f4'><th style='padding:8px;text-align:left'>회사</th><th style='padding:8px;text-align:left'>일자</th><th style='padding:8px;text-align:left'>발행유형</th><th style='padding:8px;text-align:right'>주식수</th><th style='padding:8px;text-align:left'>사유</th></tr>" + kind_rows + "</table>"
    else:
